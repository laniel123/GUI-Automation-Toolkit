...
def createHeader(parseDict) -> list:
    # Set up cookies dictionary with a fallback for 'PHPSESSID'
    cookies = {
        'PHPSESSID': parseDict.get('PHPSESSID', ''),  # Empty string if 'PHPSESSID' is missing
        # 'popCookie': parseDict.get('popCookie', ''),  # Uncomment and handle similarly if needed
    }
    headers = {
    ...
    data = {
        'url': '',
        'format': '',
        'token': parseDict.get('token', ''),  # Add a similar check for 'token'
    }
...
def TDL(cookies, headers, data, name) -> None:
    ...
    os.makedirs("vids", exist_ok=True)  #Ensure 'vids/' exists

    response = requests.get(linkParse[6:-10])
    with open('vids/tiktok_' + name + ".mp4", "wb") as f:
