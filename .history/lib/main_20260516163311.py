...
def setup_gui():
    ...
    # ── Browser control ───────────────────────────────────────────────────────
    _section_label(root, "BROWSER CONTROL")
    ...
    _radio(root, "Manual — use pixel coords from SetUp.py",
           control_mode, "manual")
    tk.Label(root, text="Smart works on any screen size with no coordinate setup.",
    ...
    tk.Label(multi_panel, text="Switch to your browser within 10 s after clicking Download.",
...
def _start_multi():
    ...
    mode_label = "Smart" if use_smart else "Manual"

    def job():
        _set_busy(True)
        try:
            log(f"Collecting {n} link(s) — {mode_label} mode.", INFO)
