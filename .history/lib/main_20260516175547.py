...
# ── App directory ─────────────────────────────────────────────────────────────
def _app_dir() -> str:
    path = os.path.join(os.path.expanduser("~"), "Documents", "TikTok Downloader")
    os.makedirs(path, exist_ok=True)
    return path

def _data_path() -> str:
    return os.path.join(_app_dir(), "data.txt")

def _default_output_dir() -> str:
    path = os.path.join(_app_dir(), "vids")
    os.makedirs(path, exist_ok=True)
    return path
...
def _save_config():
    data = {
        "url_x":      url_x_entry.get().strip(),
        "url_y":      url_y_entry.get().strip(),
        "btn_x":      btn_x_entry.get().strip(),
        "btn_y":      btn_y_entry.get().strip(),
...
def setup_gui():
    ...
    # ── Load saved config ─────────────────────────────────────────────────────
    cfg = _load_config()
    if cfg.get("url_x"):      url_x_entry.insert(0, cfg["url_x"])
    if cfg.get("url_y"):      url_y_entry.insert(0, cfg["url_y"])
    if cfg.get("btn_x"):      btn_x_entry.insert(0, cfg["btn_x"])
    if cfg.get("btn_y"):      btn_y_entry.insert(0, cfg["btn_y"])
    if cfg.get("output_dir"): output_dir.set(cfg["output_dir"])
